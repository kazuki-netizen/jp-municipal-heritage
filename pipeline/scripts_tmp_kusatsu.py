...
soup = BeautifulSoup(open('/Users/miyazakihitohata/bunkazai/pipeline/cache/滋賀県/kusatsu/page.html', encoding='utf-8', errors='ignore'), 'lxml')
tables = soup.find_all('table')
t = tables[2]
rows = t.find_all('tr')[1:]
url = "https://www.city.kusatsu.shiga.jp/bunka/rekishi/bunkazaiichiran.html"
out = []
for r in rows:
    ...
    if len(texts) < 7:
        continue
    no, kind, name, count, era, loc, date_s = texts[:7]
...
print(len(out))
with open('/Users/miyazakihitohata/bunkazai/pipeline/out/滋賀県/kusatsu.jsonl', 'w', encoding='utf-8') as f:
    for o in out:
        f.write(json.dumps(o, ensure_ascii=False) + "\n")
nulls = sum(1 for o in out if o['designated_date'] is None)
print('nulls', nulls)
kinds = set(o['subcategory'] for o in out)
print(kinds)
