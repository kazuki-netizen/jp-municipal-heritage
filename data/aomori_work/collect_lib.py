...
PARTS = ROOT / "data" / "aomori_parts"
WORK = ROOT / "data" / "aomori_work"
...
def can_fetch(url):
    ...
    if parsed.netloc not in _ROBOTS:
        ...
        try:
            ...
            # Treat an unreachable robots.txt as no published restriction.
            rp.parse([])
...
def wareki(s):
    ...
    if m:
        return f"{int(m.group(1)):04d}-{int(m.group(2)):02d}-{int(m.group(3)):02d}"
    m = re.search(r"(明治|大正|昭和|平成|令和)(元|\d+)年?(\d{1,2})月?(\d{1,2})日?", str(s))
    if m:
        g, y, mo, d = m.group(1), m.group(2), int(m.group(3)), int(m.group(4))
        y = 1 if y == "元" else int(y)
        return f"{GENGO[g] + y:04d}-{mo:02d}-{d:02d}"
...
def normalize_category(subcategory):
    c = subcategory or ""
    if any(k in c for k in ["史跡", "名勝", "天然記念物", "記念物"]):
        return "記念物(史跡・名勝・天然記念物)"
    if "無形民俗" in c or "民俗芸能" in c or "風俗慣習" in c:
        return "民俗文化財"
    if "有形民俗" in c or "民俗" in c:
        return "民俗文化財"
    if "無形" in c or "古武道" in c:
        return "無形文化財"
    if any(k in c for k in ["有形", "建造物", "彫刻", "工芸", "絵画", "絵図", "書跡", "典籍", "古文書", "考古", "歴史資料"]):
        return "有形文化財"
    return "その他"
