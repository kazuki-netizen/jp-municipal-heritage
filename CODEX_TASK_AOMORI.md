# Task: Collect 市町村指定文化財 for Aomori Prefecture (青森県)

Collect municipality-designated cultural properties for the 40 municipalities of 青森県, one part file per municipality, into `data/aomori_parts/<romaji>.jsonl`.

**Head start:** 青森市のオープンデータCSVが data/aomori_aomori_opendata.csv に取得済み（85行）。まずこれを検証してパースすること。

## Output schema (one JSON object per line; match data/iwate.jsonl exactly)

```json
{"pref":"青森県","municipality":"青森市","name":"...","category":"有形文化財","subcategory":"彫刻","designation":"市指定","designated_date":"1966-10-18","location":"...","description":null,"source_url":"https://...","source_format":"html","fetched_at":"2026-07-03T20:00:00+09:00"}
```
...
- When all municipalities are done: merge all part files into `data/aomori.jsonl`, validate, and write `notes/coverage-aomori.md` — a per-municipality table (municipality | source URL | format | rows | missed estimate | difficulty | note) in the same style as `notes/coverage.md`, plus totals and 国/県 counts observed.
