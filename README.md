...
**v13: This release adds the rest of the Tokai region (岐阜県・愛知県・三重県)
— 49,747 designations across 823 municipalities (24 prefectures).** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v13)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **49,747** | **823 / 950** |
...
- **49,747 designations total** across twenty-four prefectures (Tohoku 6 + Hokkaido + Kanto 7 + Koshinetsu-Hokuriku 6 + Tokai 4).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v13 added 9,607 IDs across Gifu/Aichi/Mie; 0 existing IDs changed).
...
```bash
python3 site/build_pages.py           # ~49,747 pages under site/p/
```
...
**v13: 東海の残り3県（岐阜・愛知・三重）を追加しました。計49,747件、823市区町村、24都道府県。**
...
| 合計 | `data/all.geojson` | **49,747** | 823 / 950市区町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v13で東海3県9,607件を追加、既存IDは全件引き継ぎ（変更0件）。
