...
**v10: This release covers all six Tohoku prefectures plus 北海道 and six Kanto prefectures (栃木県・群馬県・茨城県・埼玉県・千葉県・神奈川県)
— 23,641 designations across 457 municipalities (13 prefectures).** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v10)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **23,641** | **457 / 543** |
...
| [`data/all.geojson`](data/all.geojson) | Combined geojson across all thirteen prefectures (Tohoku + Hokkaido + six Kanto prefectures). |
...
- **23,641 designations total** across thirteen prefectures (Tohoku 6 + Hokkaido + six Kanto prefectures).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v10 added 1,644 Kanagawa IDs using JMH-14xxxx prefix; 0 existing IDs changed).
- **Coverage**: 457 municipalities yielded records. Zero-yield municipalities are listed
...
```bash
python3 site/build_pages.py           # ~23,641 pages under site/p/
```
...
**v10: 東北6県（青森・岩手・宮城・秋田・山形・福島）＋北海道＋関東6県（栃木・群馬・茨城・埼玉・千葉・神奈川）が対象です。計23,641件、457市町村、13都道府県。**
...
| 合計 | `data/all.geojson` | **23,641** | 457 / 543市町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v10で神奈川県1,644件追加（JMH-14xxxx形式）、既存IDは全件引き継ぎ（変更0件）。
