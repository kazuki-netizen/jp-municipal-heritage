...
**v8: This release covers all six Tohoku prefectures plus 北海道, 栃木県, 群馬県, 茨城県, and 埼玉県: 青森県・岩手県・宮城県・秋田県・山形県・福島県・北海道・栃木県・群馬県・茨城県・埼玉県
— 19,768 designations across 378 municipalities (11 prefectures).** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v8)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **19,768** | **378 / 510** |
...
| [`data/all.geojson`](data/all.geojson) | Combined geojson across all eleven prefectures (Tohoku + Hokkaido + Tochigi + Gunma + Ibaraki + Saitama). |
...
- **19,768 designations total** across eleven prefectures (Tohoku 6 + Hokkaido + Tochigi + Gunma + Ibaraki + Saitama).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v8 added 4,237 Saitama IDs using JMH-11xxxx prefix; 0 existing IDs changed).
- **Coverage**: 378 municipalities yielded records. Zero-yield municipalities are listed
...
```bash
python3 site/build_pages.py           # ~19,768 pages under site/p/
```
...
**v8: 東北6県（青森・岩手・宮城・秋田・山形・福島）＋北海道＋栃木県＋群馬県＋茨城県＋埼玉県が対象です。計19,768件、378市町村、11都道府県。**
...
| 合計 | `data/all.geojson` | **19,768** | 378 / 510市町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v8で埼玉県4,237件追加（JMH-11xxxx形式）、既存IDは全件引き継ぎ（変更0件）。
