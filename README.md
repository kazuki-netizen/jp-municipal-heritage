...
— 77,694 designations across 1,355 municipalities. This release also folds in gap-fill improvements for 滋賀・大阪・兵庫・奈良・和歌山・岡山 (475 rows; all previously published IDs unchanged).** The schema is designed to scale nationally
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **77,694** | **1,355 / 1,741** |
...
- **77,694 designations total** across all forty-seven prefectures of Japan — national coverage complete.
...
```bash
python3 site/build_pages.py           # ~77,694 pages under site/p/
```
...
**v17: 全国完成 — 九州・沖縄8県（福岡・佐賀・長崎・熊本・大分・宮崎・鹿児島・沖縄）を追加し、47都道府県すべてを収録しました。計77,694件、1,355市区町村。近畿・岡山のギャップ補完475件も反映（既存IDは全件不変）。**
...
| 合計 | `data/all.geojson` | **77,694** | 1,355 / 1,741市区町村 |
