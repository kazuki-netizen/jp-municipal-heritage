...
**v6: This release covers all six Tohoku prefectures plus 北海道, 栃木県, and 群馬県: 青森県・岩手県・宮城県・秋田県・山形県・福島県・北海道・栃木県・群馬県
— 13,268 designations across 274 municipalities.** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v6)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **13,268** | **274 / 466** |
...
| [`data/all.geojson`](data/all.geojson) | Combined geojson across all nine prefectures (Tohoku + Hokkaido + Tochigi + Gunma). |
...
- **13,268 designations total** across nine prefectures (Tohoku 6 + Hokkaido + Tochigi + Gunma).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v6 added 2,025 Gunma IDs using JMH-10xxxx prefix; 0 existing IDs changed).
- **Coverage**: 243 municipalities yielded records. Zero-yield municipalities are listed
...
```bash
python3 site/build_pages.py           # ~11,243 pages under site/p/
```
...
**v6: 東北6県（青森・岩手・宮城・秋田・山形・福島）＋北海道＋栃木県＋群馬県が対象です。計13,268件、274市町村。**
...
| 合計 | `data/all.geojson` | **13,268** | 274 / 466市町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v6で群馬県2,025件追加（JMH-10xxxx形式）、既存IDは全件引き継ぎ（変更0件）。
