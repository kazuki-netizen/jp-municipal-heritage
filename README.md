...
**v5: This release covers all six Tohoku prefectures plus 北海道 and 栃木県: 青森県・岩手県・宮城県・秋田県・山形県・福島県・北海道・栃木県
— 11,243 designations across 243 municipalities.** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v5)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **11,243** | **243 / 431** |
...
| [`data/all.geojson`](data/all.geojson) | Combined geojson across all eight prefectures (Tohoku + Hokkaido + Tochigi). |
...
- **11,243 designations total** across eight prefectures (Tohoku 6 + Hokkaido + Tochigi).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v5 added 1,457 Tochigi IDs; 0 existing IDs changed).
...
**v5: 東北6県（青森・岩手・宮城・秋田・山形・福島）＋北海道＋栃木県が対象です。計11,243件、243市町村。**
...
| 合計 | `data/all.geojson` | **11,243** | 243 / 431市町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v5で栃木県1,457件追加、既存IDは全件引き継ぎ（変更0件）。
