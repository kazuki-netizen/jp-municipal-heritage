...
**v14: This release adds the complete Kinki region (滋賀県・京都府・大阪府・兵庫県・奈良県・和歌山県)
— 57,368 designations across 976 municipalities (30 prefectures).** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v14)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **57,368** | **976 / 1,133** |
...
- **57,368 designations total** across thirty prefectures (Tohoku 6 + Hokkaido + Kanto 7 + Koshinetsu-Hokuriku 6 + Tokai 4 + Kinki 6).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v14 added 7,621 IDs across the Kinki region; 0 existing IDs changed).
...
```bash
python3 site/build_pages.py           # ~57,368 pages under site/p/
```
...
**v14: 近畿6府県（滋賀・京都・大阪・兵庫・奈良・和歌山）を追加しました。計57,368件、976市区町村、30都道府県。**
...
| 合計 | `data/all.geojson` | **57,368** | 976 / 1,133市区町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v14で近畿6府県7,621件を追加、既存IDは全件引き継ぎ（変更0件）。
