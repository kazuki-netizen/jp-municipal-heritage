...
**v11: This release covers all six Tohoku prefectures, 北海道, and the complete Kanto region (栃木県・群馬県・茨城県・埼玉県・千葉県・神奈川県・東京都)
— 26,630 designations across 519 municipalities (14 prefectures). Tokyo adds the 区指定 (special-ward-designated) designation type.** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v11)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **26,630** | **519 / 605** |
...
- **26,630 designations total** across fourteen prefectures (Tohoku 6 + Hokkaido + Kanto complete).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v11 added 2,993 Tokyo IDs using JMH-13xxxx prefix; 0 existing IDs changed).
...
```bash
python3 site/build_pages.py           # ~26,630 pages under site/p/
```
...
**v11: 東北6県＋北海道＋関東1都6県（栃木・群馬・茨城・埼玉・千葉・神奈川・東京）が対象です。計26,630件、519市区町村、14都道府県。関東完結。東京都は特別区の「区指定」区分を含みます。**
...
| 合計 | `data/all.geojson` | **26,630** | 519 / 605市区町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v11で東京都2,993件追加（JMH-13xxxx形式）、既存IDは全件引き継ぎ（変更0件）。
