...
**v12: This release adds 甲信越・北陸 (新潟県・山梨県・長野県・富山県・石川県・福井県) and 静岡県 on top of Tohoku, 北海道, and the complete Kanto region
— 40,140 designations across 712 municipalities (21 prefectures).** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v12)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **40,140** | **712 / 825** |
...
- **40,140 designations total** across twenty-one prefectures (Tohoku 6 + Hokkaido + Kanto 7 + Koshinetsu-Hokuriku 6 + Shizuoka).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v12 added 13,510 IDs across 7 new prefectures; 0 existing IDs changed).
...
```bash
python3 site/build_pages.py           # ~40,140 pages under site/p/
```
...
**v12: 甲信越・北陸6県（新潟・山梨・長野・富山・石川・福井）と静岡県を追加しました。計40,140件、712市区町村、21都道府県。**
...
| 合計 | `data/all.geojson` | **40,140** | 712 / 825市区町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v12で7県13,510件を追加、既存IDは全件引き継ぎ（変更0件）。
