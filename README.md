...
**v7: This release covers all six Tohoku prefectures plus 北海道, 栃木県, 群馬県, and 茨城県: 青森県・岩手県・宮城県・秋田県・山形県・福島県・北海道・栃木県・群馬県・茨城県
— 15,531 designations across 315 municipalities (10 prefectures).** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v7)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **15,531** | **315 / 510** |
...
| [`data/all.geojson`](data/all.geojson) | Combined geojson across all ten prefectures (Tohoku + Hokkaido + Tochigi + Gunma + Ibaraki). |
...
- **15,531 designations total** across ten prefectures (Tohoku 6 + Hokkaido + Tochigi + Gunma + Ibaraki).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v7 added 2,263 Ibaraki IDs using JMH-08xxxx prefix; 0 existing IDs changed).
- **Coverage**: 315 municipalities yielded records. Zero-yield municipalities are listed
...
```bash
python3 site/build_pages.py           # ~15,531 pages under site/p/
```
...
**v7: 東北6県（青森・岩手・宮城・秋田・山形・福島）＋北海道＋栃木県＋群馬県＋茨城県が対象です。計15,531件、315市町村、10都道府県。**
...
| 合計 | `data/all.geojson` | **15,531** | 315 / 510市町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v7で茨城県2,263件追加（JMH-08xxxx形式）、既存IDは全件引き継ぎ（変更0件）。
