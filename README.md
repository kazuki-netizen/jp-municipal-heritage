...
**v16: This release adds the complete Shikoku region (徳島県・香川県・愛媛県・高知県)
— 67,401 designations across 1,140 municipalities (39 prefectures).** The schema is designed to scale nationally
(see [`SCHEMA.md`](SCHEMA.md)).

## Coverage at a glance (v16)
...
| **合計** | [`data/all.geojson`](data/all.geojson) | **67,401** | **1,140 / 1,335** |
...
- **67,401 designations total** across thirty-nine prefectures (adding the Shikoku 4).
- **jmh_id**: all rows carry a unique `JMH-XXXXXX-NNNN` identifier; all prior IDs are preserved unchanged (v16 added 4,092 IDs across the Shikoku region; 0 existing IDs changed).
...
```bash
python3 site/build_pages.py           # ~67,401 pages under site/p/
```
...
**v16: 四国4県（徳島・香川・愛媛・高知）を追加しました。計67,401件、1,140市区町村、39都道府県。**
...
| 合計 | `data/all.geojson` | **67,401** | 1,140 / 1,335市区町村 |

- **jmh_id**: 全行に `JMH-XXXXXX-NNNN` 形式の固有IDを付与。v16で四国4県4,092件を追加、既存IDは全件引き継ぎ（変更0件）。
