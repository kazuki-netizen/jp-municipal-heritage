# Task: Collect 市町村指定文化財 for FUKUSHIMA Prefecture (福島県)

Collect municipality-designated cultural properties for the 59 municipalities of 福島県, one part file per municipality, into `data/fukushima_parts/<romaji>.jsonl`.

**Lessons from prior prefectures:** check municipal open-data CSV FIRST (栗原市 pattern gave a perfect official match); process municipalities strictly serially; for garbled multi-column PDFs use data/pdf_queue/ instead of writing corrupted rows.

## Output schema (one JSON object per line; match data/iwate.jsonl exactly)

```json
{"pref":"福島県","municipality":"福島市","name":"...","category":"有形文化財","subcategory":"彫刻","designation":"市指定","designated_date":"1966-10-18","location":"...","description":null,"source_url":"https://...","source_format":"html","fetched_at":"2026-07-03T20:00:00+09:00"}
```
...
- When all municipalities are done: merge all part files into `data/fukushima.jsonl`, validate, and write `notes/coverage-fukushima.md` — a per-municipality table (municipality | source URL | format | rows | missed estimate | difficulty | note) in the same style as `notes/coverage.md`, plus totals and 国/県 counts observed.
